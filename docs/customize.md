...
### Extended Regex
BH uses Python's re regex engine, but it also adds some additional back references to aid in the creation of bracket patterns.

Python's `re` module, doesn't support the following references: `\p`, `\P`, `\u`, `\U`, `\l`, `\L`, `\Q` or `\E`.  BH adds support for these by default.
...
#### Using Backrefs in BracketHighlighter Plugin
